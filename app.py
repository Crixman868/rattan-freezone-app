...
def generate_html_document(title, inv_no, date, client, c_addr, supplier, s_profile, bl, total_ctns, df, total_val, freight=None, additional_notes="", payment_terms="", signatory_position="", is_packing=False, is_duties=False, duty_data=None):
    ...
    if is_packing:
        ...
        return f'<html><body><h2>{title}</h2><p>Invoice: {inv_no}</p><p>Total Customs Bill Due: ${duty_data.get("grand_total_ttd",0):,.2f} TTD</p></body></html>'
...
def display_html_preview(raw_html):
    components.html(f'<div style="background-color: white; padding: 40px; color: #333333;">{raw_html}</div>', height=750, scrolling=True)
...
def render_admin_tracker():
    ...
    row_data = df_current[df_current['Row_UID'].astype(str).str.strip() == active_shell_uid.strip()].iloc[0] if not df_current[df_current['Row_UID'].astype(str).str.strip() == active_shell_uid.strip()].empty else {}
    ...
    with col1:
        ...
        bl_number = st.text_input("B/L Number", value=get_val("Bill of Lading Scan"))
        freight_cost = st.number_input("Ocean Freight", value=float(to_decimal(get_val("Freight", 0))))
        signatory = st.text_input("Signatory", value=get_val("Signatory", "Authorized Director"))
    ...
    with col2:
        ...
        with t_car:
            ...
            if st.button("⚙️ Preview CARICOM"):
                s_profile = get_entity_profile("suppliers.csv", "SomeSupplierName") # Adjust logic for fetching current profile
                html = generate_caricom_printout(invoice_num, invoice_date, "Client", "Supplier", "Addr", "Notes", 0, signatory, None, None, "#000000", orientation)
