...
def render_admin_tracker():
    ...
    with col1:
        ...
        freight = st.number_input("Ocean Freight", value=float(to_decimal(get_val("Freight", 0))))
        signatory = st.text_input("Signatory", value=get_val("Signatory", "Authorized Director"))
        
    with col2:
        # TABS AND PREVIEW LOGIC
        # Use get_val() for all inputs here to ensure hydration
        # ... [Add remaining hydration fields]
        st.info("Tracker Hydration Active")

# --- MAIN ROUTER ---
# [Existing Router logic remains at the bottom]
